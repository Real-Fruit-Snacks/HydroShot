...
fn hit_test_returns_edge_zones() {
    ...
    // Top edge (middle)
    assert_eq!(
        sel.hit_test(Point::new(200.0, 100.0), 8.0),
        Some(HitZone::Top)
    );
    // Bottom edge (middle)
    assert_eq!(
        sel.hit_test(Point::new(200.0, 300.0), 8.0),
        Some(HitZone::Bottom)
    );
    // Left edge (middle)
    assert_eq!(
        sel.hit_test(Point::new(100.0, 200.0), 8.0),
        Some(HitZone::Left)
    );
    // Right edge (middle)
    assert_eq!(
        sel.hit_test(Point::new(300.0, 200.0), 8.0),
        Some(HitZone::Right)
    );
}
